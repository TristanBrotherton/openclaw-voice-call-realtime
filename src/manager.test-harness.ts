...
export function createTestStorePath(): string {
  storeSeq += 1;
  // Include pid and randomness: parallel vitest workers each get their own
  // module instance, so Date.now()+counter alone can collide across workers.
  return path.join(
    os.tmpdir(),
    `openclaw-voice-call-test-${process.pid}-${Date.now()}-${storeSeq}-${Math.random().toString(36).slice(2, 8)}`,
  );
}
