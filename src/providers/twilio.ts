...
<Response>
  ...
  async playTts(input: PlayTtsInput): Promise<void> {
    ...
    // Conversation mode: speak through the live OpenAI Realtime session.
    // Falling through to TwiML <Say> would replace the active
    // <Connect><Stream> TwiML and drop the call.
    if (streamSid && this.mediaStreamHandler?.sayViaConversation(streamSid, input.text)) {
      return;
    }
...
<Response>
  ...
  async startListening(input: StartListeningInput): Promise<void> {
    // With an active media stream, transcripts arrive continuously from the
    // realtime pipeline; issuing <Gather> TwiML here would replace the
    // <Connect><Stream> and drop the call.
    if (this.mediaStreamHandler && this.callStreamMap.has(input.providerCallId)) {
      return;
    }
