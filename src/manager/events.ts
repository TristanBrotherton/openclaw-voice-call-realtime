...
// Dedupe keys only need to cover the webhook replay window; cap the set so it
// cannot grow without bound across long-lived gateway processes.
...
export function processEvent(ctx: EventContext, event: NormalizedEvent): void {
  ...
  if (ctx.processedEventIds.size > MAX_PROCESSED_EVENT_IDS) {
    ...
    let toDrop = MAX_PROCESSED_EVENT_IDS / 2;
    for (const key of ctx.processedEventIds) {
      ctx.processedEventIds.delete(key);
      if (--toDrop <= 0) {
        break;
      }
    }
