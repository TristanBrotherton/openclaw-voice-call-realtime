...
export async function findCallInStore(
  ...
  let found: CallRecord | undefined;
  for (const line of content.split("\n")) {
    if (!line.trim() || !line.includes(callIdOrProviderCallId)) {
      continue;
    }
    try {
      const call = CallRecordSchema.parse(JSON.parse(line));
      if (
        call.callId === callIdOrProviderCallId ||
        call.providerCallId === callIdOrProviderCallId
      ) {
        found = call; // last record wins
      }
    } catch {
      // Skip invalid lines.
    }
