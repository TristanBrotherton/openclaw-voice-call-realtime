...
export type VoiceResponseParams = {
  ...
  /** Latest user message */
  userMessage: string;
  /** Optional call metadata (talking points, call party, etc.) */
...
export async function generateVoiceResponse(
  ...
  const devicePolicy = resolveDevicePolicy(voiceConfig, { from });
  const devicePrompt = buildDevicePolicyPrompt(devicePolicy);

  const callContextPrompt = buildCallContextPrompt({
    callParty: params.callMetadata?.callParty as CallParty | undefined,
    talkingPoints: params.callMetadata?.talkingPoints as string[] | undefined,
  });

  const screeningPrompt =
    voiceConfig.callScreening?.enabled
      ? buildScreeningContextPrompt({
          callerIdentity: voiceConfig.callScreening.callerIdentity,
        })
      : undefined;
